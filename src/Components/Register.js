...
function Register() {
	...
	const PostData = async (e) => {
		...
		const {email,name,DOB,gender,password,cpassword,city}=userRegistration;
		...
		const res =await fetch("/Register" , {
			...
			body: JSON.stringify({
				email:email,name:name,DOB:DOB,gender:gender,password:password,cpassword:cpassword,city:city
			})
		});
		...
		<div className="screen__content2">
			...
			{/* onSubmit={handleSubmit} */}
                ...
                <div className="signup__field">
					...
					<input type="text" autoComplete="off" className="signup__input" 
					     value={userRegistration.name} onChange={handleInput} name="name" id="name" placeholder="Full Name" required/>
				</div>
                <div className="signup__field">
					<i className="signup__icon bi bi-people-fill text-secondary"></i>
